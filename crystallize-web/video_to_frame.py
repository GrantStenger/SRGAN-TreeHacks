...
def video_to_frames(video_url, res, video_name, length):
    ...
    ffmpeg_extract_subclip('data' + res + '/' + video_name + '.mp4', 0, length, targetname='data' + res + '/' + video_name + '_trimmed.mp4')
...
def download_video(yt, res, name='vid'):
    ...
    try:
        ...
        stream.download(output_path='data'+res+'/', filename=name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video_name', default='test_vid',
                        help='name of video')
    parser.add_argument('--length', default=30, type=int, help='trimmed length of video')
